...
// Parallel function to find the minimum using OpenMP with static scheduling
int find_min_parallel(int arr[], int size, int num_threads)
{
   int min_val = INT_MAX;

#pragma omp parallel for num_threads(num_threads) schedule(static) reduction(min : min_val)
   for (int i = 0; i < size; i++)
   {
      if (arr[i] < min_val)
      {
...
int main()
{
   ...
   // int min_par_2 = find_min_parallel(arr, N, 2);
   // double end_time = omp_get_wtime();
   // printf("Parallel Min for 2 threads: %d \nTime: %f seconds\n", min_par_2, end_time - start_time);

   //Parallel execution with 4 threads
   double start_time = omp_get_wtime();
   int min_par_4 = find_min_parallel(arr, N, 4);
   double end_time = omp_get_wtime();
   printf("Parallel Min for 4 threads: %d \nTime: %f seconds\n", min_par_4, end_time - start_time);
